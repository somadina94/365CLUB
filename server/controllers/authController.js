...
exports.loginUser = catchAsync(async (req, res, next) => {
  ...
  if (user.active === false) {
    return next(
      new AppError('Your account has been disabled, contact customer support.', 401)
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    const url = `https://www.365gainfuldice.com/resetPassword/${resetToken}`;
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (req.body.password !== req.body.passwordConfirm) {
    return next(
      new AppError('Your new password and confirm new password are not the same', 401)
    );
  }
